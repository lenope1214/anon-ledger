...
function buildTransaction(body, id) {
  ...
  if (moneyOnly) {
    ...
    const amount = num(body.amount != null ? body.amount : body.supplyTotal);
    ...
    return {
      tx: {
        id, companyId: resolvedMoney.companyId, date, kind, vatMode: 'none', items: [],
  ...
  if (p === '/api/companies' && m === 'GET') {
    const list = ledger.companies
      .map((c) => {
        ...
        const total = sum(['sale', 'credit_sale']);       // 물건이 나간 것
        const buyTotal = sum(['purchase', 'credit_purchase']); // 물건이 들어온 것
        return Object.assign({}, c, {
          txCount: txs.length,
          total,
          buyTotal,
          depositTotal: sum(['deposit']),
          withdrawTotal: sum(['withdraw']),
          creditSale: sum(['credit_sale']),        // 외출 (아직 정산 안 된 매출)
          creditPurchase: sum(['credit_purchase']),// 외입 (아직 정산 안 된 매입)
        });
      })
