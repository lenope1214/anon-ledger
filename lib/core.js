...
function normalizeLedger(parsed) {
  ...
  for (const t of out.transactions) {
    if (!KINDS.includes(t.kind)) t.kind = 'sale'; // 구분이 없던 예전 거래는 매출
    if (!Array.isArray(t.items)) t.items = [];
    if (!t.signed) applyComjangbuSign(t); // 컴장부 부호 규칙으로 한 번 맞춘다
  }
...
// 예전 방식(공급가액 = 수량 × 단가)으로 적힌 거래를 컴장부 부호로 한 번만 맞춘다.
//  · 물건이 나간 줄(매출·외출) → 수량 −, 금액 +
//  · 물건이 들어온 줄(매입·외입) → 수량 +, 금액 −
//  · 입금 → 금액 +, 출금 → 금액 −
// 금액의 크기는 그대로 두고 방향만 정리한다.
function applyComjangbuSign(t) {
  t.signed = true;
  if (MONEY_ONLY.includes(t.kind)) {
    const size = Math.abs(num(t.total));
    const amount = t.kind === 'deposit' ? size : -size;
    t.supplyTotal = amount;
    t.taxTotal = 0;
    t.total = amount;
    return;
  }
  const goesOut = ['sale', 'credit_sale'].includes(t.kind);
  let supplyTotal = 0;
  let taxTotal = 0;
  for (const it of t.items) {
    const qty = Math.abs(num(it.qty));
    it.qty = goesOut ? -qty : qty; // 나가면 −, 들어오면 +
    const { supply, tax } = calcItem(it.qty, num(it.price), t.vatMode);
    it.supply = supply;
    it.tax = tax;
    supplyTotal += supply;
    taxTotal += tax;
  }
  t.supplyTotal = supplyTotal;
  t.taxTotal = taxTotal;
  t.total = supplyTotal + taxTotal;
}
...
// 참조(거래 구분) 6가지 — 컴장부와 같은 이름을 쓴다
//  입금/출금: 품목 없이 금액만 (돈만 오감)
//  외출/매출: 물건이 나감 (외출 = 아직 정산 안 됨)
//  외입/매입: 물건이 들어옴 (외입 = 아직 정산 안 됨)
const KINDS = ['deposit', 'withdraw', 'credit_sale', 'sale', 'credit_purchase', 'purchase'];
const MONEY_ONLY = ['deposit', 'withdraw']; // 품목 없이 금액만 적는 구분

function buildTransaction(body, id) {
  const date = str(body.date) || todayStr();
  const kind = KINDS.includes(body.kind) ? body.kind : 'sale';
  const moneyOnly = MONEY_ONLY.includes(kind);
  const vatMode = moneyOnly ? 'none' : ['separate', 'included', 'none'].includes(body.vatMode) ? body.vatMode : 'separate';
  const items = (Array.isArray(body.items) ? body.items : [])
    .map((it) => ({ name: str(it.name), spec: str(it.spec), qty: num(it.qty), price: num(it.price) }))
    .filter((it) => it.name);
  if (!moneyOnly && !items.length) return { error: '품목을 1개 이상 입력하세요.' };
  if (moneyOnly) {
    // 입금·출금은 공급가액(금액) 한 칸만 쓴다.
    // 컴장부처럼 입금은 +, 출금은 − 로 맞춘다 (부호를 손으로 안 적어도 된다)
    const raw = num(body.amount != null ? body.amount : body.supplyTotal);
    if (!raw) return { error: '금액을 입력하세요.' };
    const amount = kind === 'deposit' ? Math.abs(raw) : -Math.abs(raw);
    const resolvedMoney = resolveCompany(body);
    if (resolvedMoney.error) return { error: resolvedMoney.error };
    return {
      tx: {
        id, companyId: resolvedMoney.companyId, date, kind, signed: true, vatMode: 'none', items: [],
        supplyTotal: amount, taxTotal: 0, total: amount, paid: 0, memo: str(body.memo),
      },
    };
  }
  ...
  return {
    tx: {
      ...
      date,
      kind,
      signed: true, // 컴장부 부호 규칙으로 계산된 거래
      vatMode,
      items,
      supplyTotal,
      taxTotal,
  ...
  if (p === '/api/companies' && m === 'GET') {
    const list = ledger.companies
      .map((c) => {
        ...
        const sum = (kinds) => txs.filter((t) => kinds.includes(t.kind)).reduce((s, t) => s + t.total, 0);
        const total = sum(['sale', 'credit_sale']);                     // 물건이 나간 것 (받을 돈 +)
        const buyTotal = Math.abs(sum(['purchase', 'credit_purchase'])); // 물건이 들어온 것 (크기로)
        return Object.assign({}, c, {
          txCount: txs.length,
          total,
          buyTotal,
          depositTotal: sum(['deposit']),
          withdrawTotal: Math.abs(sum(['withdraw'])),
          creditSale: sum(['credit_sale']),        // 외출 (아직 정산 안 된 매출)
          creditPurchase: Math.abs(sum(['credit_purchase'])), // 외입 (아직 정산 안 된 매입)
        });
      })
